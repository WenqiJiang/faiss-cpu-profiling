...
def draw_profiling_plot(
    x_labels, y_stage_1_2, y_stage_3, y_stage_4, y_stage_5, y_stage_6, y_other, filename,
    x_tick_rotation=45):
    ...
    y_all = np.array(y_stage_1_2) + np.array(y_stage_3) + \
        np.array(y_stage_4) + np.array(y_stage_5) + np.array(y_stage_6) + np.array(y_other)
    for y in y_all:
    ...
    rects_stage_4 = ax.bar(x, y_stage_4, width, bottom=bottom_stage_4)
    ...
    ax.legend([rects_stage_1_2, rects_stage_3, rects_stage_4, rects_stage_5, rects_stage_6, rects_other], 
        ["Stage 1~2: OPQ + vector quantizer", "Stage 3: select centroids", "Stage 4: construct distance LUT", \
    ...
    number_single_bar(rects_stage_4, bottom_stage_4, annotate_threshold=20)
