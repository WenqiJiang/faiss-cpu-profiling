...
def filter_events_after_timestamp(events, time_bias_start, time_bias_end):
    """
    ...
    Return: the events between (t_start + time_bias_start, t_start_bias_end)
    """
    ...
    for i in range(len(events)):
        ...
        if t >= t_start + time_bias_start:
            i_start = i
            break
    for i in range(i_start, len(events)):
        _, t = get_tid_timestamp_from_event(events[i])
        if t >= t_start + time_bias_end:
            i_end = i
            break
...
def classify_events_by_stages(events, track_non_faiss_func=True):
    ...
    for fname, time in time_consumption_all_threads_arr:
        ...
        if "::knn_L2sqr" in fname or \
            ...
            "::fvec_norm_L2sqr_ref" in fname or \
            "::precompute_list_tables" in fname:
            t_4 += time
...
faiss::fvec_inner_product_ref (/data/faiss-cpu-profiling/build/demos/demo_sift1M_read_index)
    ...
    # time_bias_start = 24.701
    # time_bias_end = 109.128
